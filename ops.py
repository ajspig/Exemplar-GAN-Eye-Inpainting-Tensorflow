...
def instance_norm(input, scope="instance_norm"):
    ...
    with tf.compat.v1.variable_scope(scope):
        ...
        scale = tf.compat.v1.get_variable("scale", [depth], initializer=tf.random_normal_initializer(1.0, 0.02)) # removed , dtype=tf.float32
        ...
        epsilon = 1e-5
        inv = 1.0 / tf.sqrt(variance + epsilon)
...
def weight_normalization(weight, scope='weight_norm'):
  ...
  with tf.compat.v1.variable_scope(scope):
    ...
    g = tf.compat.v1.get_variable('g_scalar', shape=g_shape, initializer = tf.ones_initializer())
...
def de_conv(input_, output_shape,
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        w = tf.compat.v1.get_variable('w', [k_h, k_w, output_shape[-1], input_.get_shape()[-1]], dtype=tf.float32,
                            initializer=tf.random_normal_initializer(stddev=stddev))
        try:
        ...
        # Support for verisons of TensorFlow before 0.7.0
            ...
            deconv = tf.nn.deconv2d(input_, w, output_shape=output_shape,
                                    strides=[1, d_h, d_w, 1])
        biases = tf.compat.v1.get_variable('biases', [output_shape[-1]], tf.float32, initializer=tf.constant_initializer(0.0))
...
def fully_connect(input_, output_size, scope=None, stddev=0.02, spectural_normed=True,
  ...
  with tf.compat.v1.variable_scope(scope or "Linear"):
    ...
    bias = tf.compat.v1.get_variable("bias", [output_size], tf.float32,
...
def Residual(x, output_dims=256, kernel=3, strides=1, residual_name='resi'):
    ...
    with tf.compat.v1.variable_scope('residual_{}'.format(residual_name)) as scope:
...
def spectral_norm(w, iteration= 1):
    ...
    u = tf.compat.v1.get_variable("u", [1, w.shape.as_list()[-1]], initializer=tf.compat.v1.truncated_normal_initializer(), trainable=False)
    u_hat = u
    v_hat = None
